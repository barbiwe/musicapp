    ...
    return (
        ...
        >
            <View style={styles.vinylContainer}>
                {itemCoverUrl ? (
                    <Image source={{ uri: itemCoverUrl }} style={styles.innerCover} resizeMode="cover" />
                ) : (
...
export default function PlayerScreen({ navigation, route }) {
    ...
    const handleRecommendationPlay = (track) => {
        ...
        setQueue(prevQueue => {
            // 1️⃣ Беремо history строго по currentIndex
            const history = prevQueue.slice(0, currentIndex + 1);

            // 2️⃣ Прибираємо дублікат якщо був
            const cleaned = history.filter(
                t => (t.id || t._id) !== trackId
            );
        ...
        // 3️⃣ Індекс завжди просто +1
        setCurrentIndex(prev => prev + 1);

        setRecommendations(prev =>
            prev.filter(t => (t.id || t._id) !== trackId)
        );
    };
    ...
    const handleItemLikeToggle = async (track) => {
        ...
        // Якщо це поточний трек - оновлюємо і велику кнопку лайка в хедері
        if (trackId === (currentTrack.id || currentTrack._id)) {
    ...
    const handlePlayNext = () => {
        // Спочатку закриваємо меню, щоб було видно повідомлення!
        closeModal();
        ...
        // Тепер повідомлення виїде на головному екрані
    ...
    const playFromQueue = (track) => {
        ...
        const index = queue.findIndex(t => (t.id || t._id) === id);
    ...
    const handleClearQueue = () => {
        closeModal();
        // Лишаємо тільки поточний трек, все інше видаляємо
