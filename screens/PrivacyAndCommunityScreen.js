...
export default function PrivacyAndCommunityScreen({ navigation }) {
    const [icons, setIcons] = useState({});

    // Стейт для перемикачів (початкові значення як на макеті)
    const [publicPlaylists, setPublicPlaylists] = useState(false);
    const [followersSubs, setFollowersSubs] = useState(false);
    const [playlistsVisibility, setPlaylistsVisibility] = useState(false);
    ...
    // Приймає масив description, щоб можна було робити відступи між абзацами.
    const SettingItem = ({ title, description, value, onToggle }) => (
        <View style={styles.settingItem}>
            <View style={styles.settingTextContainer}>
                <Text style={styles.settingTitle}>{title}</Text>

                {/* Якщо description - це масив, рендеримо кожен елемент як окремий абзац */}
                    ...
                    description.map((text, index) => (
                        <Text
                            ...
                            style={[
                                ...
                                index > 0 && { marginTop: scale(10) } // Відступ тільки для другого+ абзацу
                            ]}
                        >
                            {text}
                        </Text>
        ...
        <View style={styles.container}>
            ...
            >
                <SafeAreaView style={styles.safeArea}>
                    ...
                    >
                        {/* ПІДЗАГОЛОВОК 1 */}
                        <Text style={styles.sectionHeader}>Playlist visibility</Text>

                        {/* СПИСОК НАЛАШТУВАНЬ */}
                        <SettingItem
                            title="Public playlists"
                            // 👇 Передаємо масив для двох абзаців з відступом
                            description={[
                                "New playlists you create will be visible to others. They can be added to your profile.",
                                "Applies only to playlists created after changing this setting."
                            ]}
                            value={publicPlaylists}
                            onToggle={setPublicPlaylists}
                        />

                        {/* ПІДЗАГОЛОВОК 2 */}
                        <Text style={[styles.sectionHeader, { marginTop: scale(10) }]}>Profile visibility</Text>

                        <SettingItem
                            title="Followers and subscriptions"
                            description="In your profile, users can view your followers and your subscriptions."
                            value={followersSubs}
                            onToggle={setFollowersSubs}
                        />

                        <SettingItem
                            title="Playlists"
                            description="People can see the playlists you've added to your profile."
                            value={playlistsVisibility}
                            onToggle={setPlaylistsVisibility}
                        />
...
const styles = StyleSheet.create({
    ...
    header: {
        ...
        marginTop: scale(10),
        marginBottom: scale(60), // Відступ як на макеті
    },
    backButton: {
    ...
    settingDescription: {
        ...
        color: '#F5D8CB', // Колір тексту як на макеті
        lineHeight: scale(18),
    },
    ...
    switchTrack: {
        ...
        padding: scale(2),
        marginTop: scale(2), // Опускаємо перемикач на рівень Title
    },
    switchTrackActive: {
